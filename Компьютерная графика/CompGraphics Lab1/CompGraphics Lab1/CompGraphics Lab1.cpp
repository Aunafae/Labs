...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevlnstance, LPSTR lpszCmdLine, int nCmdShow) {
    ...
    setZoom(hwnd, currentZoom, ZM_DONT_KEEP);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND:
        if ((HWND)lParam == hwndButtonLine)
            ...
            if (!zoomStack.empty()) {
                setZoom(hwnd, zoomStack.top(), ZM_DONT_KEEP);
                zoomStack.pop();
            }
        break;
    case WM_CLOSE:
...
void setZoom(HWND hwnd, MyPrim newZoom, bool keep) {
    ...
    zoomScale = min(max(max(abs(currentZoom.x2 - currentZoom.x1), abs(currentZoom.y2 - currentZoom.y1)), 0.02), 1);
