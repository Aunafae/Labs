        ...
        try:
            ...
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                for teacher_data in data:
                    ...
                    teachers.append(teacher)
            update_teacher_codes()
            messagebox.showinfo("Успех", "Файл успешно открыт")
...
def exit():
    if messagebox.askokcancel("Подтверждение выхода", "Вы уверены, что хотите выйти?"):
        root.quit()

def reference_info():
    messagebox.showinfo("Справочная информация",
                        "ФИО: Банных Мария Алексеевна\nФакультет: ИПАИТ\nКурс: 3\nГруппа: 21 ПГ\n")
...
menu_bar.add_cascade(label="Справка", command=reference_info)
...
next_button = Button(frame1, text='Далее', padx=400, pady=15, font=font, command=next_command, bg=button_color, activebackground=button_color, fg="white")
next_button.pack(anchor="n", padx=20, pady=20)
